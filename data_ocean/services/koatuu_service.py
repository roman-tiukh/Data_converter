...
class KoatuuConverter(Converter):
    ...
    def create_dictionary_for_city_table(self):#table_medel is one model from ratu_models
        koatuu_dict = {}
        table_model_objects = City.objects.all()
        for table_record in table_model_objects: # dictionary for keeping whole model class objects
            if (table_record.name):
    ...
    #creating a dictionary out off the values, which already exists in district y citydistrict table
    def create_dictionary_for_citydistrict_table(self, table_medel):#table_medel is one model from ratu_models
        koatuu_dict = {}
        table_model_objects = table_medel.objects.all()
        for table_record in table_model_objects: # dictionary for keeping whole model class objects
            if (table_record.name):
    ...
    def save_to_db(self,data):
        ...
        district_dict = self.create_dictionary_for_citydistrict_table(District)
        city_dict = self.create_dictionary_for_city_table()
        citydistrict_dict = self.create_dictionary_for_citydistrict_table(Citydistrict)
        #getting values in json file Koatuu
        for index, object_koatuu in enumerate(data):
            if not (object_koatuu[self.LEVEL_ONE]!='') & (object_koatuu[self.LEVEL_TWO]==''):
                return
            else:
            ...
            if not (object_koatuu[self.LEVEL_ONE]!='') & (object_koatuu[self.LEVEL_TWO]!='') & (object_koatuu[self.LEVEL_THREE]==''):
                return
            else:
            ...
            if not (object_koatuu[self.LEVEL_ONE]!='') & (object_koatuu[self.LEVEL_TWO]!='') & (object_koatuu[self.LEVEL_THREE]!='') & (object_koatuu[self.LEVEL_FOUR]==''):
                return
            else:
                ...
                self.save_to_citydistrict_table(data, object_koatuu, object_level_three, citydistrict_dict, object_city_name, object_region_name, region_dict)
            if not (object_koatuu[self.LEVEL_ONE]!='') & (object_koatuu[self.LEVEL_TWO]!='') & (object_koatuu[self.LEVEL_THREE]!='') & (object_koatuu[self.LEVEL_FOUR]!=''):
                return
            else:
                ...
                self.save_to_citydistrict_table(data, object_koatuu, object_level_four, citydistrict_dict, object_citydistrict_name, object_region_name, region_dict)
    ...
    def save_to_city_table(self, data, object_koatuu, object_level_number, district_dict, city_dict, object_level_name, district_items_list):
        for object_district_name in district_items_list:
            if not (object_district_name in district_dict):
                return
            else:
                district_koatuu = district_dict[object_district_name]
    ...
    def save_to_citydistrict_table(self, data, object_koatuu, object_level_number, citydistrict_dict, object_level_name, object_region_name, region_dict):     
        if not (object_region_name in region_dict):
            return
        else:
            region_koatuu = region_dict[object_region_name]            
            if not (str(object_level_number)[2]=='1') & (str(object_level_number)[:2] == str(region_koatuu.koatuu)[:2]):
                return
            else:
                object_level_name = object_level_name + str(region_koatuu.id)
